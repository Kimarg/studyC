...
void displayStatus(int *stock, int *restocked, int *sold) {
    ...
    if (maxSoldID != -1) printf("가장 많이 판매된 상품: ID %d, 판매량 %d\n", maxSoldID, maxSold);
    if (minSoldID != -1) printf("가장 적게 판매된 상품: ID %d, 판매량 %d\n", minSoldID, minSold);

    for (int i = 0; i < PRODUCT_COUNT; i++) {
        if (stock[i] == 0) printf("상품 ID %d: 재고부족(0)\n", i);
    }
}
...
int main() {
    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            displayStatus(stock, restocked, sold);
        } else if (choice == 4) {
            printf("프로그램을 종료합니다.\n");
            break;
        } else {
